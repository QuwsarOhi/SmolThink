...
def r1_distill(reasoning_len, tokenizer, n_data=None):
...
def general_thought(reasoning_len, tokenizer, n_data=None):
    def generalreason_conv(data):
        ...
        data['source'] = "GeneralReasoning/GeneralThought-430K"
        return data


    genreason_dataset = load_dataset("GeneralReasoning/GeneralThought-430K")["train"]
    ...
    genreason_dataset = genreason_dataset.filter(
        lambda x: len(x["model_reasoning"].strip()) < reasoning_len
    ...
    if n_data:
        genreason_dataset = genreason_dataset.select(range(n_data))
    print("General reason dataset length (after filter):", len(genreason_dataset))
    return genreason_dataset


def smoltalk(data_split, tokenizer, n_data=None):
    ...
    smoltalk_data = concatenate_datasets([smoltalk_data['train'], smoltalk_data['test']])
    col_names = smoltalk_data.column_names
    smoltalk_data = smoltalk_data.map(data_process, fn_kwargs={"source": f"HuggingFaceTB/smoltalk/{data_split}"})
    smoltalk_data = smoltalk_data.remove_columns(col_names)
    if n_data:
        smoltalk_data = smoltalk_data.shuffle(123).select(range(n_data))

    print(f"Smoltalk/{data_split} dataset length (after filter):", len(smoltalk_data))
...
if __name__ == '__main__':
    TOKENIZER_PATH = "HuggingFaceTB/SmolLM2-360M-Instruct"
    REASONING_LEN = 386
    CONTEXT_LEN_LIMIT = None # 1024

    tokenizer = get_tokenizer(TOKENIZER_PATH)
    dataset = concatenate_datasets([
        r1_distill(reasoning_len=REASONING_LEN, tokenizer=tokenizer),
        general_thought(reasoning_len=REASONING_LEN, tokenizer=tokenizer),
        smoltalk(data_split="systemchats-30k", tokenizer=tokenizer),
        smoltalk(data_split="everyday-conversations", tokenizer=tokenizer)
    ]).shuffle(123)

    dataset = dataset.map(lambda data: {**data, 'ctx_len': len(tokenizer.encode(data['conversations']))})
    print("Total dataset length:", len(dataset), flush=True)
    dataset.to_json(f"datasets/dataset_cot{REASONING_LEN}.jsonl", orient="records")
...
def smoltalk_fcall(tokenizer, sys_template, n_data=None):
    ...
    if n_data: smoltalk_fc_dataset = smoltalk_fc_dataset.select(range(n_data))
